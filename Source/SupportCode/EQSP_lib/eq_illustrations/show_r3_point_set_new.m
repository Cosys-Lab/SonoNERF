function show_r3_point_set_new(points_x,color,varargin)
...
% options, which are specified as eRTIS_Sensor_Overlay, value pairs.
...
r = min(0.05,N^(-1/2)/2)*10;
rX = r*X; rY = r*Y; rZ = r*Z;
for n = 1:N
   surf(points_x(1,n)+rX,points_x(2,n)+rY,points_x(3,n)+rZ,color(n)*ones(size(rZ)),...
   'FaceColor','interp','FaceLighting','none','EdgeColor','none')
